...
def _random_unit(seed: int, *parts: object) -> float:
    ...
    return random.Random(int.from_bytes(hashlib.sha256(payload).digest()[:8], "big")).random()
...
class WorkflowEngine:
    ...
    def run(self, task: JoinTask, profile: RoleProfile, mode: RunMode, seed: int) -> WorkflowRun:
        ...
        if violation:
            if mode in {RunMode.CONTRACT_CHECK, RunMode.STATIC_VERIFIER}:
                ...
                plan = self._plan(task, profile, mode, seed, attempt=1, force_complete=True)
                cost.planner_calls += 1
                recoveries.append(
                    RecoveryEvent(
                ...
                plan = self._plan(task, profile, mode, seed, attempt=1, force_complete=False)
                cost.planner_calls += 1
                recoveries.append(
                    RecoveryEvent(
        ...
        return WorkflowRun(task, mode, plan, solution, review, handoffs, recoveries, cost)
    ...
    def _plan(
        ...
        omit = not force_complete and _random_unit(seed, task.task_id, "planner", attempt) < omission_rate
        if omit:
    ...
    def _solve(self, task: JoinTask, plan: PlanArtifact, profile: RoleProfile, seed: int) -> SolverArtifact:
        ignores_plan = _random_unit(seed, task.task_id, "solver") < profile.solver_ignore_plan_rate
    ...
    def _review(self, task: JoinTask, solution: SolverArtifact, profile: RoleProfile, seed: int) -> ReviewArtifact:
        mismatch = solution.output_rows != task.expected_rows
        detects = mismatch and _random_unit(seed, task.task_id, "reviewer") < profile.reviewer_detection_rate
        return ReviewArtifact(detected_mismatch=detects, checklist_complete=solution.preserves_row_semantics)
