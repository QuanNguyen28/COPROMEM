...
def plan_cardinality_present(event: HandoffEvent, contract: "Contract") -> tuple[bool, str]:
...
SCOPE_GUARDS: dict[str, ScopeGuard] = {"join_preservation_scope": join_preservation_scope}
...
class Contract:
    ...
    def storage_tokens(self) -> int:
        return self.estimated_read_tokens + sum(len(item.split()) for item in self.counterexamples)
    ...
    def without_required_field(self, field_name: str) -> "Contract":
        return replace(self, required_fields=tuple(item for item in self.required_fields if item != field_name))
...
def contract_from_failure(failed_run: "WorkflowRun", successful_run: "WorkflowRun") -> Contract | None:
...
def contracts_for_interface(contracts: Iterable[Contract], interface: str) -> list[Contract]:
