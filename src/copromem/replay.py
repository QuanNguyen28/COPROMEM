...
def compare_at_checkpoint(
    ...
    for task in task_list:
        results = {mode: engine.run(task, profile, mode, seed) for mode in (
            RunMode.NO_MEMORY,
            RunMode.TEXT_RULE,
            RunMode.SHAM_RETRY,
        ...
        if not results[RunMode.NO_MEMORY].success and results[RunMode.CONTRACT_CHECK].success:
            beneficial += 1
        if results[RunMode.NO_MEMORY].success and not results[RunMode.CONTRACT_CHECK].success:
            harmful += 1
...
def minimize_by_clause_deletion(
    ...
    for field_name in reversed(tuple(current.required_fields)):
        ...
        if evidence.net_gain_over_text >= min_gain and evidence.harmful_flips <= harm_cap:
