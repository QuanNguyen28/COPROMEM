...
def collect_build_experiences(client: Any, build: list[Example]) -> tuple[list[TaskRun], list[dict[str, Any]]]:
    runs: list[TaskRun] = []
...
def _format_experiences(runs: list[TaskRun], plans: list[dict[str, Any]]) -> str:
    ...
    for run, plan in zip(runs, plans):
        items.append(
            ...
            "TRAJECTORY: planner artifact=" + json.dumps(plan, sort_keys=True) +
            f"; predicted={run.predicted}\nOUTCOME: {'SUCCESS' if run.correct else 'FAILURE'}"
        )
    return "\n\n".join(items)


def induce_expel(client: Any, runs: list[TaskRun], plans: list[dict[str, Any]]) -> CallResult:
    prompt = "Extract generally applicable insights from these experiences:\n\n" + _format_experiences(runs, plans)
...
def induce_awm(client: Any, runs: list[TaskRun], plans: list[dict[str, Any]]) -> CallResult:
    successful = [(run, plan) for run, plan in zip(runs, plans) if run.correct]
    selected = successful or list(zip(runs, plans))
    text = _format_experiences([item[0] for item in selected], [item[1] for item in selected])
    return client.chat(AWM_INDUCTION_SYSTEM, "Extract reusable math workflows:\n\n" + text, max_tokens=420)


def memory_plan_call(client: Any, example: Example, label: str, memory: str) -> tuple[dict[str, Any], CallResult]:
    user = (
...
def _eval_expr(node: ast.AST, values: dict[str, Any]) -> Any:
    ...
    if isinstance(node, ast.BinOp) and type(node.op) in _BINARY:
        return _BINARY[type(node.op)](_eval_expr(node.left, values), _eval_expr(node.right, values))
    ...
    if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in _FUNCTIONS:
        ...
        return _FUNCTIONS[node.func.id](*[_eval_expr(item, values) for item in node.args])
...
def safe_execute_arithmetic(text: str) -> tuple[Decimal | None, str]:
    ...
    except (SyntaxError, ValueError, TypeError, ZeroDivisionError, OverflowError, InvalidOperation) as exc:
        return None, f"{type(exc).__name__}: {exc}"


def run_critic_task(client: Any, example: Example) -> tuple[TaskRun, dict[str, Any]]:
    initial = client.chat(CRITIC_PROGRAM_SYSTEM, f"Question: {example.question}", max_tokens=400)
    ...
    if prediction is None and initial_prediction is not None:
        ...
        final_report = final_report + "; retained prior executable answer per CRITIC evaluator"
    ...
    return (
        ...
        {
            "initial_prediction": str(initial_prediction) if initial_prediction is not None else None,
...
def _compact(runs: list[TaskRun], details: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    result = []
...
def run_literature_benchmark(
    ...
    arm_runs["expel"] = [run_memory_task(client, item, "expel", expel_memory.text) for item in evaluation]
    checkpoint("expel")
    arm_runs["agent_workflow_memory"] = [
        run_memory_task(client, item, "agent_workflow_memory", awm_memory.text) for item in evaluation
    ]
    ...
    checkpoint("critic")
    arm_runs["copromem"] = [
        run_task(client, item, "copromem", success_memory, bool(contract["admitted"]))[0]
    ...
    all_usages = [usage for run in build_runs for usage in run.calls] + induction_usages + [
    ...
    return {
        ...
        "implementation_provenance": {
            "repositories": {
                name: {"url": REPO_URLS[name], "revision": revisions[name]} for name in REPO_URLS
            },
        ...
        "memory_construction": {
            "shared_build_collection": {"metrics": build_metrics, "tasks": _compact(build_runs)},
            "expel": {"memory": expel_memory.text, "usage": _usage_dict([expel_memory.usage])},
            "agent_workflow_memory": {"memory": awm_memory.text, "usage": _usage_dict([awm_memory.usage])},
        ...
        "evaluation": {
            name: {
                ...
                "tasks": _compact(arm_runs[name], critic_details if name == "critic" else None),
            }
            for name in ARMS
        },
        "paired_vs_copromem": {
...
def reconcile_report(
    ...
    if rerun_expel:
        ...
        expel_runs = [run_memory_task(client, item, "expel", expel_memory.text) for item in evaluation]
        report["evaluation"]["expel"] = {"metrics": summarize_runs(expel_runs), "tasks": _compact(expel_runs)}
        ...
        added_usages = [usage for run in rebuilt_runs for usage in run.calls] + [expel_memory.usage]
        added_usages += [usage for run in expel_runs for usage in run.calls]
    else:
        report["validity"] = report.get("validity", {})
        report["validity"]["expel"] = "excluded: induction returned provider-null content (literal None)"
    ...
    for task in critic_tasks:
        ...
        if predicted is not None:
            ...
            try:
                ...
                if not correct:
                    parsed_errors.append(float(abs(Decimal(str(predicted)) - Decimal(str(task["gold"])))) )
    ...
    critic_metrics["task_specific"]["final_plan_schema_pass_rate"] = parsed_count / count
    ...
    critic_metrics["cost"]["usd_per_success"] = total_cost / correct_count if correct_count else None
    critic_metrics["agent_system"]["recovered_task_success_rate"] = correct_count / count

    def task_map(name: str) -> dict[str, bool]:
        return {item["id"]: bool(item["correct"]) for item in report["evaluation"][name]["tasks"]}
    ...
    for name in ("expel", "agent_workflow_memory", "critic"):
        ...
        beneficial = sum(not baseline[item] and copromem_correct[item] for item in copromem_correct)
        harmful = sum(baseline[item] and not copromem_correct[item] for item in copromem_correct)
        paired[name] = {"beneficial_flips": beneficial, "harmful_flips": harmful, "net_gain": beneficial - harmful}
    ...
    report["experiment_totals"]["reconciled_agent_calls"] = report["experiment_totals"]["measured_agent_calls"] + len(added_usages)
    report["experiment_totals"]["reconciled_measured_cost_usd"] = report["experiment_totals"]["measured_cost_usd"] + sum(item.usd for item in added_usages)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark literature baselines against CoProMem")
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/gsm8k_literature_seed42_20test.json"))
    parser.add_argument("--repair-report", type=Path, default=None, help="Reconcile a completed report's provider-null ExpeL/CRITIC outputs")
    parser.add_argument("--local-reconcile", action="store_true", help="Only reconcile saved CRITIC outputs; make no provider calls")
    ...
    if args.repair_report and args.local_reconcile:
        report = reconcile_report(client, args.repair_report, args.output, [], [], rerun_expel=False)
        for arm in ARMS:
            metrics = report["evaluation"][arm]["metrics"]
            print(f"{arm}: accuracy={metrics['task_success']['pass_at_1']:.3f}, calls/task={metrics['efficiency']['calls_per_task']:.2f}, cost=${metrics['cost']['total_usd']:.6f}")
    ...
    if args.repair_report:
        report = reconcile_report(client, args.repair_report, args.output, build, evaluation, rerun_expel=not args.local_reconcile)
        for arm in ARMS:
            metrics = report["evaluation"][arm]["metrics"]
            print(f"{arm}: accuracy={metrics['task_success']['pass_at_1']:.3f}, calls/task={metrics['efficiency']['calls_per_task']:.2f}, cost=${metrics['cost']['total_usd']:.6f}")
    ...
    report = run_literature_benchmark(
        client, build, evaluation, client.model_metadata(), args.vendor_root, checkpoint_path
    )
    ...
    print(f"Total: {totals['measured_agent_calls']} calls, ${totals['measured_cost_usd']:.6f}; wrote {args.output}")
