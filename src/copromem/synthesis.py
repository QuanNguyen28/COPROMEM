...
from .types import JoinIntent, JoinTask, RoleProfile, RunMode, ScopeEvidence, WorkflowRun
...
def localize_handoff_failures(runs: Iterable[WorkflowRun]) -> list[WorkflowRun]:
    ...
    return [run for run in runs if not run.success and run.plan.declared_cardinality is None]


def propose_contracts(runs: Iterable[WorkflowRun], max_per_failure: int = 3) -> list[Contract]:
...
def evaluate_scope_and_counterexamples(
    ...
    contract_runs = [engine.run(task, transfer_profile, RunMode.CONTRACT_CHECK, seed) for task in in_scope]
    no_memory = [engine.run(task, transfer_profile, RunMode.NO_MEMORY, seed) for task in boundary]
    boundary_contract = [engine.run(task, transfer_profile, RunMode.CONTRACT_CHECK, seed) for task in boundary]
    eligible = [run for run in contract_runs if run.handoffs[0].verifier_results]
    harmful = sum(old.success and not new.success for old, new in zip(no_memory, boundary_contract))
    ...
    return ScopeEvidence(
        ...
        in_scope_success=sum(run.success for run in contract_runs) / max(1, len(in_scope)),
        boundary_harmful_flips=harmful,
        veto_accuracy=sum(vetoes) / max(1, len(boundary)),
        transfer_success=sum(run.success for run in contract_runs) / max(1, len(in_scope)),
    )
