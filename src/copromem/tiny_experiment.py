...
def run_tiny_experiment(seed: int = 7) -> dict[str, object]:
    ...
    build_runs = run_many(WorkflowEngine(), splits["build"], SOURCE_PROFILE, RunMode.NO_MEMORY, seed)
    ...
    final_tasks = [
        ...
        if task.group_id.endswith(("00", "01")) and task.intent is JoinIntent.PRESERVE_ROWS
    ]
    arms = {
        ...
        RunMode.CONTRACT_CHECK.value: run_many(
            WorkflowEngine(bank), final_tasks, SOURCE_PROFILE, RunMode.CONTRACT_CHECK, seed
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the tiny three-arm CoProMem experiment")
    parser.add_argument("--seed", type=int, default=7)
    parser.add_argument("--output", type=Path, default=Path("artifacts/tiny_experiment.json"))
    ...
    print("Success rates: " + ", ".join(f"{name}={rate:.3f}" for name, rate in rates.items()))
