...
def summarize(runs: Sequence[WorkflowRun]) -> ArmMetrics:
    ...
    return ArmMetrics(
        ...
        macro_group_success=sum(sum(values) / len(values) for values in group_successes.values()) / max(1, len(group_successes)),
        ...
        recovery_success_rate=(sum(event.successful for event in recovery_events) / len(recovery_events)) if recovery_events else 0.0,
    )


def paired(reference: Sequence[WorkflowRun], treatment: Sequence[WorkflowRun]) -> PairedMetrics:
    if [run.task.task_id for run in reference] != [run.task.task_id for run in treatment]:
        raise ValueError("paired comparison requires task-aligned runs")
    beneficial = sum(not old.success and new.success for old, new in zip(reference, treatment))
    harmful = sum(old.success and not new.success for old, new in zip(reference, treatment))
...
def clustered_bootstrap_success_delta(
    ...
    for _ in range(samples):
        ...
        deltas.append(sum(new_values) / len(new_values) - sum(old_values) / len(old_values))
    deltas.sort()
