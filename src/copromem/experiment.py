...
def build_contract_bank(seed: int) -> tuple[ContractBank, dict[str, object]]:
    ...
    build_runs = run_many(
        collection_engine, splits["build"], SOURCE_PROFILE, RunMode.NO_MEMORY, seed
    )
    candidates = propose_contracts(build_runs)
    if not candidates:
        raise RuntimeError(
            "synthetic build set did not yield an observable handoff candidate"
        )
    ...
    minimized, minimized_replay = minimize_by_clause_deletion(
        candidate, splits["dev"], SOURCE_PROFILE, seed
    )
    boundaries = [
        task for task in splits["audit"] if task.intent.value == "intentional_expansion"
    ]
    scope_tasks = [
        task for task in splits["dev"] if task.requires_cardinality_rationale
    ]
    scope = evaluate_scope_and_counterexamples(
        minimized, scope_tasks, boundaries, TRANSFER_PROFILE, seed
    )
    bank = ContractBank()
    decision = bank.admit(minimized, minimized_replay, scope)
    if not decision.admitted:
        raise RuntimeError(
            "candidate unexpectedly failed admission: " + "; ".join(decision.reasons)
        )
    ...
    lifecycle = CostLedger(
        ...
        replay_cost=float(
            replay.checkpoint_count * 4 + minimized_replay.checkpoint_count * 4
        ),
...
def run_experiment(seed: int = 7) -> dict[str, object]:
    ...
    return {
        ...
        "final_source": {
            name: as_jsonable(summarize(runs)) for name, runs in source_arms.items()
        },
        "final_transfer_executor_swap": {
            name: as_jsonable(summarize(runs)) for name, runs in transfer_arms.items()
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the reproducible CoProMem synthetic pilot"
    )
    parser.add_argument("--seed", type=int, default=7)
    parser.add_argument(
        "--output", type=Path, default=Path("artifacts/synthetic_report.json")
    )
    ...
    print(
        f"Admitted {contract_count} contract(s). Final success: no-memory={baseline:.3f}, contract={source:.3f}"
    )
