...
from copromem.experiment import SOURCE_PROFILE, TRANSFER_PROFILE, build_contract_bank, run_experiment
...
def test_contract_recovers_an_incomplete_planner_handoff() -> None:
    ...
    no_memory = next(run for run in build_runs if not run.success and run.plan.declared_cardinality is None)
    ...
    repaired = WorkflowEngine(bank).run(no_memory.task, SOURCE_PROFILE, RunMode.CONTRACT_CHECK, seed=7)
...
def test_counterexample_is_vetoed() -> None:
    ...
    boundary = next(task for task in grouped_split()["final"] if task.intent is JoinIntent.INTENTIONAL_EXPANSION)
    run = WorkflowEngine(bank).run(boundary, TRANSFER_PROFILE, RunMode.CONTRACT_CHECK, seed=7)
...
def test_end_to_end_pilot_improves_over_simple_baseline() -> None:
    ...
    assert source["contract_check"]["success_rate"] > source["text_rule"]["success_rate"]
